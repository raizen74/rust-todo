...
pub async fn login(
    ...
    println!(
        "login(address {}, user_id {}, timeout_mins {}, perm_user_id {})",
        address, user_id, timeout_mins, perm_user_id
    );
...
pub async fn update(address: &str, user_id: &str) -> Result<UserSessionStatus, NanoServiceError> {
    println!("update(address {}, user_id {})", address, user_id);
    let mut con = get_connnection(address).await?;
    let result = con
    ...
    println!("perm_user_id {}", perm_user_id);
